...
DATE_FMT = "%Y%m%d%H%M"
ROWS_PER_PAGE = 100
MAX_PAGES = 50                  # safety cap (30→50 확대)


# ---------------------------------------------------------------------
# G2B API fetch
# ---------------------------------------------------------------------
def fetch_notices(days_back: int) -> Tuple[List[Dict], int]:
    """Fetch all notices posted within the last ``days_back`` days. Paginate
    until exhaustion or MAX_PAGES cap. Returns (notices, pages_used)."""
    end_dt = datetime.now()
...
def passes_filter(n: Dict) -> bool:
    ...
    # budget range — null 허용 (수의계약·예가 미공개 공고 누락 방지).
    # null일 때 통과시키되, 값이 있으면 범위 검증.
    bmin = n.get("bid_amount_min")
    bmax = n.get("bid_amount_max")
    if bmin is not None and bmin < BUDGET_MIN:
        return False
    if bmax is not None and bmax > BUDGET_MAX:
        return False
...
def _render_stats_footer(stats: Dict) -> str:
    """모니터링용 fetch 통계 — 메일 맨 아래 작게. 사용자(소르)가 시스템 동작 자가 점검."""
    return f"""
    <div style="margin-top:32px;padding:10px 14px;background:#f9f9f9;border-top:1px solid #eee;color:#888;font-size:11px;">
      📊 <b>Fetch 통계</b> ·
      API fetch: <b>{stats.get('fetched', 0):,}</b>건 ·
      필터 통과: <b>{stats.get('passed', 0)}</b>건 ·
      (신규 {stats.get('new_count', 0)} / 재공고 {stats.get('repost_count', 0)}) ·
      페이지 사용: {stats.get('pages_used', '?')}/{MAX_PAGES} ·
      실행: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    </div>
    """


def render_digest(new_list: List[Dict], repost_list: List[Dict], stats: Optional[Dict] = None) -> str:
    ...
    <html>
      <body style="font-family:-apple-system,BlinkMacSystemFont,'Apple SD Gothic Neo',sans-serif;color:#222;max-width:780px;margin:0 auto;">
        ...
        {sections}
        {_render_stats_footer(stats) if stats else ""}
      </body>
    </html>
    """
...
def main() -> int:
    ...
    notices, pages_used = fetch_notices(DAYS_BACK)
    logger.info("Fetched %d total notices (pages_used=%d)", len(notices), pages_used)
    ...
    html_body = render_digest(new_list, repost_list, stats=stats)
